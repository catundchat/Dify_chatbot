...
当我在 Dify 数据集里放同一本书籍的不同格式文件（如txt, pdf）后，发现由此数据集索引构建的应对书籍中很多问题回答得更准确，对书籍文本的掌握程度更高。我的理解是：对于一本书的多种形式 pdf, txt，用户查询 query 的向量在进行相似度搜索后，能够匹配到两种格式书籍中对应的文段。而这些都将作为之后传给 ChatGPT 的输入，也就是说增加了 ChatGPT 上下文中关于用户查询语句最相似的部分（可能重复），从而起到了回答更准确的作用。
